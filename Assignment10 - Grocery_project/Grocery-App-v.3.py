            ...
            if x == 1:  # Display Product list
                ...
                for i, j in cart.items():
                    for x, y in Grocery_prod_dict.items():
                        if i == x:
                            price_cart1.update(
                                {i: j * int(y['Individual_Price'])})  # Used for adding key and individual price x qty in price_dict1
                print(price_cart1)  # Had to create a new dict as old one was not updating(need to check why?)
